...
import { Home, BarChart2, MessageSquare, Target, PlusCircle } from 'lucide-react';
...
export function BottomNav({ activeView, onViewChange }: Props) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-brand-medium/95 backdrop-blur border-t border-brand-light flex items-center justify-around py-2 px-1">
      {items.map(item => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center gap-1 px-2 py-1.5 rounded-lg transition-all ${active ? 'text-brand-purple' : 'text-gray-500'}`}
          >
